...
if app_mode == nav_home:
    st.markdown(f'<h2 class="main-header">{translate_text("🌿 PLANT DISEASE RECOGNITION SYSTEM",target_language)}</h2>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="div">{translate_text(home_text, target_language)}</div>', unsafe_allow_html=True)
